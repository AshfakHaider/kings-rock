...
create table public.stock_accounts (
  ...
  notes text,
  zeusx_enabled boolean not null default false,
  zeusx_status text not null default 'pending' check (zeusx_status in ('pending', 'posting', 'posted', 'failed')),
  zeusx_category text,
  zeusx_game text,
  zeusx_server text,
  zeusx_delivery_method text,
  zeusx_delivery_days integer not null default 0 check (zeusx_delivery_days >= 0),
  zeusx_delivery_hours integer not null default 1 check (zeusx_delivery_hours >= 0),
  zeusx_description text,
  zeusx_tags text[] not null default '{}',
  zeusx_listing_url text,
  zeusx_posted_at timestamptz,
  zeusx_error text,
...
before update on public.stock_accounts
for each row execute function public.guard_stock_accounts_zeusx_admin_update();
...
create index stock_accounts_zeusx_pending_idx
...
create index advance_transactions_advance_id_idx on public.advance_transactions(advance_id);
create unique index employee_advances_request_id_idx on public.employee_advances(request_id) where request_id is not null;
